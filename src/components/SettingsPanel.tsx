        ...
        <div ref={modalRef} className="relative bg-white dark:bg-slate-800 rounded-xl shadow-xl max-w-md w-full animate-slide-up">
          ...
          <div className="px-6 py-4 space-y-6 max-h-[70vh] overflow-y-auto">
            ...
            <div className="border-t border-gray-200 dark:border-slate-700 pt-6">
              ...
              {/* Enable Suspicious Transaction Detection Toggle */}
              <div className="mt-4">
                <label className="flex items-center justify-between cursor-pointer">
                  <div>
                    <span className="text-sm font-medium text-gray-700 dark:text-gray-300">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                      Warn about duplicates, large amounts, and unusual patterns
                    </p>
                  </div>
                  <button
                    type="button"
                    role="switch"
                    aria-checked={localSettings.enableSuspiciousDetection}
                    onClick={() => setLocalSettings({ ...localSettings, enableSuspiciousDetection: !localSettings.enableSuspiciousDetection })}
                    className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 dark:focus:ring-offset-slate-800 ${
                      localSettings.enableSuspiciousDetection ? 'bg-primary-600' : 'bg-gray-200 dark:bg-slate-600'
                    }`}
                  >
                    <span
                      className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                        localSettings.enableSuspiciousDetection ? 'translate-x-5' : 'translate-x-0'
                      }`}
                    />
                  </button>
                </label>
